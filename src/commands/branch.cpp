...
void branch(const std::string& name){
  ...
  std::string refs_path = gollumDir() + "/.gollum/refs/" ;
...
void printBranches(){
  ...
  std::string refs_path = repo_root+ "/.gollum/refs/";
  std::ifstream in(repo_root+"/.gollum/HEAD");
  if(!in){
  ...
  if(line.rfind("ref: ",0) == 0){
    ...
    current_branch = std::filesystem::relative(repo_root+"/.gollum/"+branch_path,refs_path+"/heads/");
  }
  else{
