...
std::string gollumDir(){
    std::filesystem::path current = std::filesystem::current_path();

    while (true) {
        if (std::filesystem::exists(current / ".gollum") && std::filesystem::is_directory(current / ".gollum")) {
        ...
        if (current.has_parent_path() && current.string() != "/") {
            ...
            throw std::runtime_error("Not inside an gollum repository.");
        }
    }
    return current;
}
...
std::unordered_set<std::string> loadIgnoreFiles(){
  std::string repo_root = gollumDir();
...
std::unordered_set<std::string> loadIgnoreFile(const std::string& path) {
    ...
    std::string file = path + "/.gollumignore";
...
std::string createTempFile() {
    std::string temp_file = std::filesystem::temp_directory_path() / "gollum_commit_msg.txt";
...
std::unordered_map<std::string,std::unordered_map<std::string,std::vector<std::string>>> loadConfig(){
  ...
  std::string repo_root = gollumDir();
  std::string local_config_file = repo_root + "/.gollum/config";
  std::string user_config_file1 = "~/.gollumconfig";
  std::string user_config_file2 = "~/config/gollum/config";
