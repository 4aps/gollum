...
std::string getHead(){
  ...
  std::string head_path = repo_root + "/.gollum/HEAD";
  ...
  if(line.rfind("ref: ",0) == 0){
    ...
    std::ifstream branch_file(repo_root + "/.gollum/" + branch_path);
    if(!branch_file){
...
void updateHead(const std::string& branch_name){
  std::string repo_root = gollumDir();
  std::string head_file = repo_root + "/.gollum/HEAD";
...
void setHead(const std::string& commit_hash){
  ...
  if(line.rfind("ref: ",0) == 0){
    std::string branch_path = line.substr(5);
    std::ofstream branch_file(repo_root + "/.gollum/" + branch_path,std::ios::binary);
    if(!branch_file){
      std::cerr<<"error opening branch file";
      return;
    }
...
std::string getBranchHash(const std::string& branch_name){
  std::string repo_root = gollumDir();
  std::string branch_path = repo_root + "/.gollum/refs/heads/" + branch_name;
